...
export default function LoginForm() {
  ...
  const onLoginSubmit = async (data: FieldValues) => {
    ...
    try {
      ...
      if (!callback?.ok) {
        toast.error(callback?.error || "Identifiants incorrects. Veuillez vérifier votre email et mot de passe.");
        return;
      }
            ...
            <form onSubmit={handleSubmit(onLoginSubmit)} className="space-y-6">
              <div>
                ...
                <Input
                  ...
                  className={`appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-blueviolet focus:border-blueviolet focus:z-10 sm:text-sm ${errors.email ? 'border-red-500' : ''
                    }`}
                />
                {errors.email && (
              ...
              <div>
                ...
                <div className="relative">
                  <Input
                    ...
                    className={`appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-blueviolet focus:border-blueviolet focus:z-10 sm:text-sm ${errors.password ? 'border-red-500' : ''
                      }`}
                  />
                  <button
                    type="button"
              ...
              <div className="flex items-center justify-end">
                <div className="text-sm">
                  <Link href="/forgot-password" className="font-medium text-blueviolet hover:text-blueviolet/80">
                    Mot de passe oublié ?
                  </Link>
                </div>
              </div>
